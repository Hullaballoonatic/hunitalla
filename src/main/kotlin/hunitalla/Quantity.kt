...
abstract class Quantity<Q : Quantity<Q>>(value: Double, name: String?, val unit: Unit<Q>, val dimension: Dimension) :
    ...
    override val value = unit.baseValueOf(value)
    ...
    operator fun times(unit: Unit<*>): Quantity<*> = (dimension * unit).baseUnit(value)
    operator fun div(unit: Unit<*>): Quantity<*> = (dimension / unit).baseUnit(value)

    operator fun div(divisor: Number) = div(divisor.toDouble())
    operator fun div(divisor: Double) = of(value / divisor)

    operator fun times(other: Quantity<*>): Quantity<*> = (dimension * other).baseUnit(value * other.value)

    operator fun div(other: Quantity<*>): Quantity<*> = (dimension / other).baseUnit(value / (other.value))

    infix fun pow(exponent: Int) = (dimension pow exponent).baseUnit(value)
    infix fun root(base: Int) = (dimension root base).baseUnit(value)
